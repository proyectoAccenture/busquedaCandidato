...
public interface IPostulationRepository extends JpaRepository<PostulationEntity, Long> {
    List<PostulationEntity> findAllByCandidateId(Long candidateId);
    ...
    Page<PostulationEntity> findByStatusIn(List<Status> statuses, Pageable pageable);
    ...
    @Query("""
    SELECT p FROM PostulationEntity p
    JOIN p.candidate c
    JOIN c.origin o
    ...
    WHERE p.status IN :statuses AND (
    ...
    OR CAST(COALESCE(c.card, 0) AS string) LIKE LOWER(CONCAT('%', :query, '%'))
    )
""")
    Page<PostulationEntity> searchByCandidateNameLastNameAndRole(
            @Param("word1") String word1,
            @Param("word2") String word2,
            @Param("word3") String word3,
            @Param("word4") String word4,
